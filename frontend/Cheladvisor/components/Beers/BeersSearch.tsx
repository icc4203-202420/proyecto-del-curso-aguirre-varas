            ...
            <View>
              <TouchableOpacity onPress={() => handleBeerClick(item.id)}>
                <Card containerStyle={{ padding: 10 }}>
                  ...
                  <Text>Rating: {item.avg_rating !== null ? item.avg_rating : "N/A"}</Text>
                  ...
                  {expandedBeerId === item.id && (
                    <View>
                      ...
                      <View style={{ flexDirection: "row", marginBottom: 10 }}>
                        {[1, 2, 3, 4, 5].map((star) => (
                          <TouchableOpacity key={star} onPress={() => setRating(star)}>
                            <Text style={{ fontSize: 20, color: star <= rating ? "gold" : "lightgray" }}>★</Text>
                          </TouchableOpacity>
                        ))}
                      </View>
                      ...
                      <Button title="Submit Review" onPress={() => handleSubmitReview(item.id)} />
                    </View>
                  )}
                </Card>
              </TouchableOpacity>
